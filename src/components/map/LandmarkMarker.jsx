...
function LandmarkMarker({ mapBounds, landmark, onSelect }) {
  ...
  const position = useMemo(() => {
    ...
    const longitudeRatio = (landmark.longitude - COORDINATE_BOUNDS.lonMin) /
      (COORDINATE_BOUNDS.lonMax - COORDINATE_BOUNDS.lonMin)
    const latitudeRatio = (landmark.latitude - COORDINATE_BOUNDS.latMin) /
    ...
    const y = mapBounds.min.y + (mapBounds.max.y - mapBounds.min.y) * 0.01 + (landmark.yOffset ?? 0)

    return [x, y, z]
  }, [landmark.latitude, landmark.longitude, landmark.zIndex, landmark.yOffset, mapBounds])
  ...
  useFrame(() => {
    ...
    const scaleValue = MathUtils.clamp(distance * 0.045, 0.3, 3.2) * baseScale
  ...
  return (
    <group ref={markerRef} position={position}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
        <ringGeometry args={[0.18, 0.45, 48]} />
        <meshBasicMaterial color="#fbbf24" transparent opacity={hovered ? 0.9 : 0.6} />
      </mesh>
    </group>
  )
}
